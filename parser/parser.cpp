...
namespace parser {
    node_list_t parse(token_vec_t tokens) {
        if (!tokens.empty() && tokens[0].type == token::token_type::NEWLINE)
            return parse(std::vector<token::token>(tokens.begin() + 1, tokens.end()));
    ...
    std::vector<std::vector<token::token>> split_tokens(token_vec_t tokens, unsigned int index) {
        ...
        const std::vector<token::token_type> valid_types = {
                ...
                //  Punctuation:
                //  ------------
                token::token_type::COLON, token::token_type::LCURLY, token::token_type::RCURLY,
                token::token_type::LPAREN, token::token_type::RPAREN, token::token_type::LSQUARE,
                        ...
                        [&](const std::shared_ptr<ast_node::expr_node>& node) {
                            ...
                            switch (node->type) {
                                ...
                                case ast_node::node_type::ARRAY_LOOP_EXPR:
                                    return std::reinterpret_pointer_cast<ast_node::array_loop_expr_node>(node)
                                                   ->item_expr
                                        != nullptr;
                                ...
                                case ast_node::node_type::UNOP_EXPR:
                                    return is_expr(
                                            std::reinterpret_pointer_cast<ast_node::unop_expr_node>(node)->operand);
                                case ast_node::node_type::BINOP_EXPR:
                                    return is_expr(std::reinterpret_pointer_cast<ast_node::binop_expr_node>(node)
                                                           ->right_operand);
                                case ast_node::node_type::IF_EXPR:
                                    if_node = std::reinterpret_pointer_cast<ast_node::if_expr_node>(node);
                                    if (if_node->negative_expr != nullptr) { return is_expr(if_node->negative_expr); }
                                    return false;
        ...
        single_op_rule parse_prefix = [&](std::vector<std::shared_ptr<ast_node::expr_node>>& expressions,
            ...
            while (true) {
                ...
                if (current_node->type == ast_node::node_type::IF_EXPR) {
                    const std::shared_ptr<ast_node::if_expr_node> if_expr
                            = std::reinterpret_pointer_cast<ast_node::if_expr_node>(current_node);
                    if (if_expr->conditional_expr == nullptr) {
                        ...
                        for (unsigned int sub_index = index + 1; sub_index < (unsigned int)expressions.size();
                             sub_index++) {
                            ...
                            if (current_expr->type == ast_node::node_type::IF_EXPR) {
                                const std::shared_ptr<ast_node::if_expr_node> inner_if_expr
                                        = std::reinterpret_pointer_cast<ast_node::if_expr_node>(current_expr);
    ...
    */
    ...
    parser_return_t parse_expr(token_vec_t tokens, unsigned int index) {
        ...
        for (const std::vector<token::token>& subset : subsets) {
    ...
    */
    ...
    parser_return_t parse_cmd_assert(token_vec_t tokens, unsigned int index) {
        ...
        const token::token& string_tok = tokens[index];
